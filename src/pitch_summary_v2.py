"""
투구 메커니즘 분석 — 동작 단계 자동 분류 탑재 버전
출력: Stride Length, Release Extension, Max Hip-Shoulder Separation (릴리스 시점 기준)

촬영 가이드:
    측면(3루 또는 1루쪽, 투구 방향과 90도) 영상 하나만 있으면 된다.
    이 각도가 보폭/익스텐션은 물론, 힙-숄더 세퍼레이션 측정에도 최적이다.
    (정면/후면에서 찍으면 골반·어깨 회전축이 카메라와 거의 평행해져
     회전각이 실제보다 작게 측정되는 문제가 있다 — 반드시 측면에서 촬영할 것)

핵심 개선점:
    이전 버전은 영상 전체에서 "정상 범위 안의 최댓값"을 찾았기 때문에,
    와인드업 중 다리를 드는 순간처럼 투구와 무관한 동작에서도 값이 잡힐 수 있었다.
    이 버전은 손목 속도/위치 패턴을 분석해 각 프레임을 6단계로 자동 분류하고,
    "release(릴리스)" 단계로 분류된 프레임에서만 HSS를 측정한다.
    보폭/익스텐션은 "투구손이 어깨보다 위로 올라간" 프레임에서만 측정해
    정지된 준비 자세를 착지/릴리스로 오인하는 문제를 막는다.

투구 동작 6단계:
    1. windup      (와인드업)   — 다리를 들어올리는 준비 동작
    2. stride       (스트라이드) — 앞발이 타겟 방향으로 뻗어나가는 중
    3. cocking      (코킹)      — 착지 직후, 팔이 뒤로 젖혀진 상태
    4. acceleration (가속)      — 손목 속도가 급격히 증가하는 구간
    5. release      (릴리스)    — 손목 속도가 최대인 순간 (공을 놓는 시점)
    6. follow_through (팔로스루) — 릴리스 이후 감속 구간
